...
- `nx_maple_res` 项目（固定路径 `../nx_maple_res`）负责原始 `.nx` 输入和导出工具链。
...
```bash
cd ../nx_maple_res
moon run --target native cmd/main -- <subcommand> [args...]
```
...
- 原始输入：`../nx_maple_res/assets/<Package>.nx`
- 导出工具：`../nx_maple_res/cmd/main`
- 中间产物目录 `../nx_maple_res/cache/nx_export` 不作为长期依赖
...
优先用 `export` 子命令一次导出可运行资源：

```bash
cd ../nx_maple_res
moon run --target native cmd/main -- export \
  --out-dir <maple-moon-root>/assets \
  --include 'Map/Map1/102000001.img' \
  assets/Map.nx
```

- `--include` 支持 `*` 和 `**` 通配，可多次指定。推荐始终使用 `--include` 精确导出，避免无关资源大面积改动。
- 导出后应在目标目录生成 `Map/.../mx.json` + `map.tmj` + `tilesets/*.tsj` + 贴图文件。
...
- `../nx_maple_res/cache/nx_export` 仍可作为排查导出问题的中间层，但默认不提交。
...
- 处理后资源需与 `src/engine/resource/` 的 `base_url` 与目录前缀一致。
