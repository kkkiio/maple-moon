...
class FtLoader {
  ...
  loadGlyph(name, h, c) {
    return new FtGlyph(this.textCtx, name, h, c)
  }
}
class FtGlyph {
  constructor(textCtx, name, h, c) {
    this.textCtx = textCtx
    this.name = name
    this.h = h
    this.c = c
    textCtx.canvas.width = 100;
    textCtx.canvas.height = 100;
    textCtx.font = `${h}px ${name}`
    this.metrics = textCtx.measureText(c)
  }
  ...
  width() {
    return this.metrics.actualBoundingBoxLeft + this.metrics.actualBoundingBoxRight
  }
  height() {
  ...
  bearingX() {
    return -this.metrics.actualBoundingBoxLeft
  }
  bearingY() {
  ...
  loadBitmap() {
    ...
    if (w === 0 || h === 0) {
      return {
        id: -1, // TODO: distinguish image and bitmap
        data: new Uint8Array(),
        w: 0,
        h: 0,
      }
    ...
    textCtx.font = `${this.h}px ${this.name}`
    textCtx.clearRect(0, 0, w, h);
    textCtx.fillText(this.c, this.metrics.actualBoundingBoxLeft, this.metrics.actualBoundingBoxAscent);
...
function main() {
  ...
  const importObject = {
    ...
    font: {
      new_ftloader: () => new FtLoader(),
      load_glyph: (loader, name, h, c) => loader.loadGlyph(name, h, c),
      width: (glyph) => glyph.width(),
      height: (glyph) => glyph.height(),
      advance_x: (glyph) => glyph.advanceX(),
      bearing_x: (glyph) => glyph.bearingX(),
      bearing_y: (glyph) => glyph.bearingY(),
      load_bitmap: (glyph) => glyph.loadBitmap(),
    },
    resource: {
      ui_loader: () => uiLoader,
